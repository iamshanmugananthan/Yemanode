...
if command -v pipx >/dev/null 2>&1; then
    ...
    if pipx list | grep -i codesentinel >/dev/null 2>&1; then
        echo "[*] Removing legacy CodeSentinel via pipx..."
        pipx uninstall codesentinel
        UNINSTALLED=1
    fi
fi
...
if command -v pip3 >/dev/null 2>&1; then
    ...
    pip3 uninstall -y codesentinel >/dev/null 2>&1 && UNINSTALLED=1 || true
elif command -v pip >/dev/null 2>&1; then
    echo "[*] Attempting pip uninstall..."
    pip uninstall -y yemanode >/dev/null 2>&1 && UNINSTALLED=1 || true
    pip uninstall -y codesentinel >/dev/null 2>&1 && UNINSTALLED=1 || true
fi
